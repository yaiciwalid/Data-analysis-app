...
dash.register_page(__name__, path='/overview', title="Plotly Deep Learning App", description="Deep learning simplified")
    ...
    html.Div([
        ...
        html.Div(
            children=[
                ...
                dcc.Textarea(
                    id="sql-input",
                    placeholder="Write your SQL query here (ex: SELECT * FROM data WHERE age > 30). Use df as name of your table",
                    style={
                        "width": "98%",
                        "height": "150px",
...
def update_table(pathname, n_clicks, data):
    if data is not None:
        try:
            ...
            return df.to_dict('records'), [{"name": col, "id": col, "type": "numeric" if pd.api.types.is_numeric_dtype(df[col]) else "text"} for col in df.columns], data
        except:
...
def execute_query(n_clicks, data,query):
    if data is not None and n_clicks>0:
        try:
            ...
            return result_df.to_dict("records"), [{"name": col, "id": col} for col in result_df.columns], result_df.to_dict("records"), False, []
        except:
            return dash.no_update, dash.no_update, dash.no_update, True, ["Error occured when executing SQL query, retry"]
    else:
        return [], [],[], True, ["No Data available, re-upload dataset"]
